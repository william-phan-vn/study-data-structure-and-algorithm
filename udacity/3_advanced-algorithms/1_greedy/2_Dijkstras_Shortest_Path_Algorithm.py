...
# TODO: Find the shortest path from the source node to every other node in the given graph '''
def dijkstra(graph, source):
    ...
    while unvisited: 
        ...
        # 6.1. Find the unvisited node having smallest known distance from the source node.
        ...
        # 6.2. For the current node, find all the unvisited neighbours.
        ...
        for neighbor in graph.neighbours[min_node]:
            if neighbor in unvisited:
                ...
                # 6.3. If the calculated distance of the unvisited neighbour
                ...
                if ((neighbor not in result) or (distance < result[neighbor])):
                    ...
                    # 6.4. If there is an update in the result dictionary,
...
def dijkstra_2(graph, source):
    # 1. Create a result dictionary. At the end of the program, result will have the shortest distance (value) for all nodes (key) in the graph.
    # For our example, it will become as {'A': 0, 'B': 5, 'C': 3, 'D': 2, 'E': 4, 'F': 6}
    result = {}

    # 2. Start with the source node. Distance from source node to source node itself is 0.
    result[source] = 0
    ...
    # 5. Create a `path` dictionary that keeps track of the previous node (value) that can lead to the current node (key).
    # At the end of the program, for our example, it will become as {'B': 'A', 'C': 'D', 'D': 'A', 'E': 'C', 'E': 'C', 'F': 'C'}
    path = {}

    '''THE GREEDY APPROACH'''
    # 6. As long as unvisited is non-empty
    while unvisited: 
        min_node = None    
